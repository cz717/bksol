...
Inductive is_bnode : Z_btree -> Prop :=
    is_bnode_intro : forall (n:Z) (t1 t2:Z_btree), is_bnode (Z_bnode n t1 t2)
 .
...
Definition naive_occ_dec : forall (n:Z) (t:Z_btree), {occ n t} + {~ occ n t}.
 ...
 case (Z_eq_dec n z).
 intro H2; elim H2; left; auto with searchtrees.
...
Inductive min (z:Z) : Z_btree -> Prop :=
  ...
  | min_node : forall z' t1 t2,  z < z' -> 
                            min z t1 -> min z t2 -> min z (Z_bnode z' t1 t2).
(*
...
Hint Resolve min_leaf min_node : searchtrees.


(* z is greater than every label in t *)
Inductive maj (z:Z) : Z_btree -> Prop :=
  | maj_leaf : maj z Z_leaf
  | maj_node : forall z' t1 t2, z > z' -> maj z t1 -> maj z t2 
                           -> maj z (Z_bnode z' t1 t2).
               
(*    maj_intro : (forall z':Z, occ z' t -> z' < z) -> maj z t
 .
*)
(*Hint Resolve maj_intro: searchtrees.*)
Hint Resolve maj_leaf maj_node : searchtrees.


Theorem min_occ : forall z t, min z t -> 
  (forall x, occ x t -> z < x).
Proof.
  intros z t H x H'.
  induction H'.
  - inversion H. assumption.
  - apply IHH'. inversion H. assumption.
  - apply IHH'. inversion H. assumption.
Qed.

Theorem maj_occ : forall z t, maj z t ->
  (forall x, occ x t -> z > x).
Proof.
intros z t H x H'.
  induction H'.
...
Theorem occ_min : forall z t, (forall x, occ x t -> z < x) -> min z t.
Proof.
  intros z t H.
  induction t; constructor; auto with searchtrees.
Qed.

Theorem occ_maj : forall z t, (forall x, occ x t -> z > x) -> maj z t.
Proof.
  intros z t H.
  induction t; constructor; auto with searchtrees.
Qed.

Hint Resolve min_occ maj_occ occ_min occ_maj : searchtrees.

(* searchness predcate on binary trees *)
...
Proof.
  intros z t H.
  induction H; unfold not; intros H'.
  - inversion H'.
  - inversion H'; auto with searchtrees zarith.

(*
 unfold not; intros z t H H'.
 elim H; intros; absurd (z < z); auto.
 apply Zlt_irrefl.
*)
Qed.
...
Section search_tree_basic_properties.
 ...
 Proof.
   intros p H H'. induction H'. 
   - cut (n < p).
     + apply Zle_not_lt; assumption.
     + inversion se. inversion H6. assumption.
   - apply IHH'. 
     inversion se. inversion H4. inversion H6.
     apply (bnode_search_tree H3 H10 H5 H18).
   - apply IHH'.
     inversion se. inversion H4. inversion H6. 
     apply bnode_search_tree; assumption.
(*
...
Qed.
 ...
 Proof.
   intros p H H'. induction H'.
   - cut (n > p).
     + apply Zle_not_gt. apply Z.ge_le. assumption.
     + inversion se. inversion H5. assumption.
   - apply IHH'. 
     inversion se. inversion H3. inversion H5.
     apply bnode_search_tree; assumption.
   - apply IHH'. 
     inversion se. inversion H3. inversion H5.
     apply bnode_search_tree; assumption.
 ...
 Proof.
  ...
  absurd (occ p t1).
  apply not_left.
  unfold Zge; rewrite H0; discriminate. 
  auto.
 Qed.
...
Proof.
 ...
 - elim H1; constructor 2; auto with searchtrees.
   ...
   + apply occ_maj. intros x H'.
     destruct (H4 x H') as [T|E].
     * apply maj_occ with (t:=t1).
       inversion H0; assumption.
       assumption.
     * apply Zlt_gt. rewrite <- E. assumption.
   + inversion H0. assumption.
(*
 intros n p t1 t'1 t2 H H0 H1; split.
 intros p0 H2; inversion_clear H2.
...
intros n p t1 t'1 t2 H H0 H1; split.
 ...
 - intros p0 H2.
   inversion_clear H2; auto with searchtrees.
   elim H1; intros. 
   elim (H5 p0); auto with searchtrees.
 - elim H1; constructor 2; auto with searchtrees.
   + eapply search_tree_l; eauto with searchtrees.
   + inversion H0. assumption.
   + apply occ_min. intros x H'.
     destruct (H4 x H') as [T|E].
     * apply min_occ with (t:=t'1).
       inversion H0; assumption.
       assumption.
     * auto with zarith.
(*
...
Proof.
 ...
 - intros p H0.
   ...
   + elim H1; intro H2.
     * apply Zlt_le_weak.  
       apply Zgt_lt. apply maj_occ with (t:=t).
...
Section RMAX_np.
     ...
     *)
     ...
     Proof.
      ...
      elim (maj_l S1). 
      intro H8.
      cut (p0 < n); auto with searchtrees.
...
Proof.
 ...
 apply rm_intro. unfold not; intro H2.
 elim (occ_inv H2).
 intro eg.  
 apply Zlt_irrefl with n.
...
Section rm_root.
     ...
     Proof.
      ...
      elim (H5 p0 H'1). intro; left; auto with searchtrees.
      intro e; case e; left; auto with searchtrees.
      left; auto with searchtrees.
     Qed.
     ...
     Proof.
      ...
      apply maj_intro.
      intros q0 H5.
      elim (Zle_lt_or_eq  _ _ (H0 q0 (H1 q0 H5))).
      auto with searchtrees.
